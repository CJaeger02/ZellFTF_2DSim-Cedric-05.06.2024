...
class MachineView(QGraphicsView):
    def __init__(self, factory: Factory, machine: Machine, parent=None) -> None:
        print('OPENED MACHINE VIEW:')
        super(MachineView, self).__init__(parent)
        ...
        """print(f'Inside CLASS MachineView - Factory Cell Size = {self.factory.cell_size}')
        print(f'Inside CLASS MachineView - Factory Length = {self.factory.length}')
        print(f'Inside CLASS MachineView - Factory Width = {self.factory.width}')
        print(f'Inside CLASS MachineView - PIXELS = {self.pixels}')
        print(f'Inside CLASS MachineView - Szene = {self.scene()}')"""
    ...
    def mousePressEvent(self, event):
        ...
        column = int(pos.x() // self.get_cell_size_for_grid(self.factory, self.machine.length, self.machine.width))
        row = int(pos.y() // self.get_cell_size_for_grid(self.factory, self.machine.length, self.machine.width))
    ...
    def add_input_clicked(self):
        """print(f'Inside CLASS MachineView - INPUT CLICKED')
        print(f'Inside CLASS MachineView - X = {self.mouse_x_pos}, Y = {self.mouse_y_pos} ,'
              f'Column = {self.column}, Row = {self.row}')
        print(f'Inside CLASS MachineView - Function add_input_clicked - BEFORE '
              f'self.machine.pos_input[0] = {self.machine.pos_input[0]}, '
              f'self.machine.pos_input[1] = {self.machine.pos_input[1]}'
              f'self.machine.local_pos_input[0] = {self.machine.local_pos_input[0]}, '
              f'self.machine.local_pos_input[1] = {self.machine.local_pos_input[1]}')"""
        self.machine.local_pos_input = [self.column, self.row]
        self.scene().delete_input_output()
        self.scene().draw_input_output()
        """print(f'Inside CLASS MachineView - Function add_input_clicked - AFTER '
              f'self.machine.pos_input[0] = {self.machine.pos_input[0]}, '
              f'self.machine.pos_input[1] = {self.machine.pos_input[1]}'
              f'self.machine.local_pos_input[0] = {self.machine.local_pos_input[0]}, '
              f'self.machine.local_pos_input[1] = {self.machine.local_pos_input[1]}')"""
    ...
    def add_output_clicked(self):
        """print(f'Inside CLASS MachineView - OUTPUT CLICKED')
        print(f'Inside CLASS MachineView - X = {self.mouse_x_pos}, Y = {self.mouse_y_pos} ,'
              f'Column = {self.column}, Row = {self.row}')
        print(f'Inside CLASS MachineView - Function add_output_clicked - BEFORE '
              f'self.machine.pos_output[0] = {self.machine.pos_output[0]}, '
              f'self.machine.pos_output[1] = {self.machine.pos_output[1]}'
              f'self.machine.local_pos_output[0] = {self.machine.local_pos_output[0]}, '
              f'self.machine.local_pos_output[1] = {self.machine.local_pos_output[1]}')"""
        self.machine.local_pos_output = [self.column, self.row]
